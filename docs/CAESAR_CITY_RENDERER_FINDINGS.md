...
- **`100F:1583`, the main text routine.**
  ...
  - It advances 8 px for `FONT1`, 6 px for `MINIFONT`, and draws through `303E:13D6`. (`MINIFONT.PL1` turned out to be 1 bit per pixel -- `docs/FORMATS.md` -- so how its bits reach that 8-bit planar blitter isn't re-checked; the same `DS:0F64` table does map onto its glyphs.)
...
Implemented in `render::render_city` (pass a save's actor table). Moving them is `systems::actors`, transcribed in `CAESAR_CONSTRUCTION_DISPATCH_FINDINGS.md` section 20. The walking frames are a base per type plus a facing set x 3 plus a stride; the bases are 0 (types 0, 8 and 10), 12, 24, 36, 48, 60 and 72 (types 1, 2, 4, 5, 6 and 7).
