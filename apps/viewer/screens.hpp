...
inline ui::Page forum_page(const model::CityState& s, ForumTab tab, int speed = -1, int hint = 0,
    ...
    for (int i = 0; i < kForumTabCount; ++i) {
        ...
        page.tabs.push_back({ui::tr(kTabs[i]), kActionTab + i});
    }
    page.selected_tab = tab < kForumTabCount ? tab : -1;
    page.buttons.push_back({ui::tr("Save"), kActionOpenSave});
    page.buttons.push_back({ui::tr("Load"), kActionOpenLoad});
    page.buttons.push_back({ui::tr("Close"), kActionClose});
    const auto dn = [](int v) { return std::to_string(v) + " Dn"; };
    switch (tab) {
        case kTreasurer: {
            page.title = ui::tr("The Treasurer") + std::string(", ") + year_text(g(s, 0x6C32));
            page.rows.push_back({ui::tr("Funds"), dn(g(s, systems::economy::kFunds))});
            page.rows.push_back(detail::control_row(s, ui::tr("Population tax"), forum::Control::PopulationTax, " %"));
            page.rows.push_back(detail::control_row(s, ui::tr("Industrial tax"), forum::Control::IndustrialTax, " %"));
            page.rows.push_back({ui::tr("City population"), std::to_string(g(s, 0x6C0E))});
            const int cents = g(s, 0x6BC8);
            page.rows.push_back({ui::tr("Tax per head"),
                                 std::to_string(g(s, 0x6BCA)) + "." + (cents < 10 ? "0" : "") + std::to_string(cents) +
                                     " Dn"});
            // FONT1 has no colon or slash (DS:0F64 draws ':' as '0', '/' as nothing).
            page.rows.push_back({ui::tr("Last year in - population tax"), dn(g(s, 0x6BC6))});
            page.rows.push_back({std::string("  ") + ui::tr("industrial tax"), dn(g(s, 0x6BC4))});
            page.rows.push_back({ui::tr("Out - construction"), dn(g(s, 0x6BC2))});
            page.rows.push_back({std::string("  ") + ui::tr("operating costs"), dn(g(s, 0x6BBE))});
            page.rows.push_back({std::string("  ") + ui::tr("tribute to Rome"), dn(g(s, 0x6BAA))});
            const int profit = static_cast<int16_t>(g(s, 0x6BB6));
            page.rows.push_back({profit < 0 ? ui::tr("Overall loss") : ui::tr("Overall profit"), dn(profit < 0 ? -profit : profit)});
            break;
        }
        case kTribune: {
            using systems::plebs::kFireNeed;
            page.title = ui::tr("The Tribune of the Plebs");
            page.rows.push_back({ui::tr("Pleb groups"), std::to_string(g(s, systems::plebs::kPlebs))});
            page.rows.push_back(detail::control_row(s, ui::tr("Welfare"), forum::Control::Welfare, " Dn"));
            page.rows.push_back(detail::duty_row(s, ui::tr("Fire prevention"), forum::Duty::FirePrevention, kFireNeed));
            page.rows.push_back(detail::duty_row(s, ui::tr("Building upkeep"), forum::Duty::BuildingMaintenance,
                                                 systems::plebs::kBuildingNeed));
            page.rows.push_back(
                detail::duty_row(s, ui::tr("Road upkeep"), forum::Duty::RoadMaintenance, systems::plebs::kRoadNeed));
            // The construction need DS:0x6C3E isn't saved (systems::plebs::set_needs).
            page.rows.push_back(detail::duty_row(s, ui::tr("Construction"), forum::Duty::Construction, 0));
            page.rows.push_back(detail::duty_row(s, ui::tr("Army duty"), forum::Duty::ArmyDuty, 0));
            page.rows.push_back({ui::tr("Auxiliary Centuries"), std::to_string(g(s, systems::military::kAuxiliaries))});
            page.rows.push_back({ui::tr("Unassigned"), std::to_string(g(s, systems::plebs::kUnassigned))});
            break;
        }
        case kLegion: {
            ...
            if (slot >= 0) {
                ...
                page.rows.push_back({ui::tr("Cohort") + std::string(" ") + std::to_string(number + 1) + ", " + emblem,
                                     cohort_state_name(a.state()), kActionPrevCohort, kActionNextCohort});
                page.rows.push_back({std::string("  ") + ui::tr("Morale"), std::to_string(a.raw[military::kCohortMorale])});
                page.rows.push_back({std::string("  ") + ui::tr("Regulars"), std::to_string(a.raw[military::kCohortRegulars])});
                page.rows.push_back({std::string("  ") + ui::tr("Irregulars"), std::to_string(a.raw[military::kCohortIrregulars])});
                page.rows.push_back({std::string("  ") + ui::tr("Auxiliaries"), std::to_string(a.raw[military::kCohortAuxiliaries])});
                page.buttons.insert(page.buttons.begin(),
                                    {a.state() == military::kCohortDemobilized ? ui::tr("Mobilize") : ui::tr("Demobilize"),
                                     kActionMobilize});
            } else {
                page.rows.push_back({ui::tr("No Cohort"), "", kActionPrevCohort, kActionNextCohort});
            }
            const auto with_last = [&](uint16_t now, uint16_t last) {
                return std::to_string(g(s, now)) + " (" + std::to_string(g(s, last)) + ")";
            };
            page.rows.push_back({ui::tr("Regular Centuries"), with_last(military::kRegulars, military::kRegularsLastYear)});
            page.rows.push_back(
                {ui::tr("Irregular Centuries"), with_last(military::kIrregulars, military::kIrregularsLastYear)});
            page.rows.push_back(
                {ui::tr("Auxiliary Centuries"), with_last(military::kAuxiliaries, military::kAuxiliariesLastYear)});
            page.rows.push_back(detail::control_row(s, ui::tr("Wages bill"), forum::Control::ArmyWages, " Dn"));
            page.rows.push_back(detail::control_row(s, ui::tr("Conscription"), forum::Control::Conscription, " %"));
            break;
        }
        case kRatings: {
            namespace admin = systems::administration;
            const int rank = g(s, admin::kRank);
            page.title = std::string(rank_name(rank)) + " " + ui::tr("of") + " " + province_name(g(s, 0x6CA6));
            page.rows.push_back({ui::tr("Peace"), std::to_string(g(s, admin::kPeace))});
            page.rows.push_back({ui::tr("Culture"), std::to_string(g(s, admin::kCulture))});
            page.rows.push_back({ui::tr("Prosperity"), std::to_string(g(s, admin::kProsperity))});
            page.rows.push_back({ui::tr("Empire"), std::to_string(g(s, admin::kEmpire))});
            page.rows.push_back({ui::tr("Average"), std::to_string(g(s, admin::kAverage))});
            if (rank >= 0 && rank < static_cast<int>(admin::kPromotion.size()) && rank < 20) {
                ...
                page.rows.push_back({ui::tr("To become") + std::string(" ") + rank_name(rank + 1), ""});
                page.rows.push_back({std::string("  ") + ui::tr("average of"), std::to_string(need.average)});
                page.rows.push_back({std::string("  ") + ui::tr("and each at least"), std::to_string(need.each)});
            }
            page.rows.push_back({ui::tr("Year"), year_text(g(s, 0x6C32))});
            ...
            for (int c = 3; c >= 0; --c) page.buttons.insert(page.buttons.begin(), {ui::tr(kAsk[c]), kActionHint + c});
            break;
        }
        case kIndustry: {
            // 0x09D22, the man in green.
            const forum::IndustryReport r = forum::industry_report(s);
            page.title = std::string(ui::tr("Industry Report on")) + " " + province_name(r.province);
            page.rows.push_back({ui::tr("Overall Industry Rating"), forum::kGradeNames[static_cast<size_t>(r.overall)]});
            page.rows.push_back({ui::tr("Prospects for Expansion"), forum::kGradeNames[static_cast<size_t>(r.prospects)]});
            page.rows.push_back({ui::tr("Industry type"), "suitability  factories"});
        ...
        case kStatue: {
            ...
            page.title = ui::tr("The statue");
            page.rows.push_back({ui::tr("Rank"), rank_name(g(s, forum::kRank)), kActionRankDown, kActionRankUp});
            break;
        }
        case kGovernor:
        case kForumTabCount: {
            page.title = ui::tr("The Governor");
            // 0x0C328: the governor's screen shows the name (DS:0x5858).
            page.rows.push_back({ui::tr("Name"), trimmed(ui::governor_name(s))});
            page.rows.push_back({ui::tr("Rank"), rank_name(g(s, systems::administration::kRank))});
            page.rows.push_back({ui::tr("Province"), province_name(g(s, 0x6CA6))});
            page.rows.push_back(detail::control_row(s, ui::tr("Salary"), forum::Control::Salary, " Dn"));
            page.rows.push_back({ui::tr("Personal savings"), dn(g(s, 0x6C2E))});
            page.rows.push_back(detail::control_row(s, ui::tr("Donation"), forum::Control::Donation, " Dn"));
            if (speed >= 0) page.rows.push_back({ui::tr("Game speed"), std::to_string(speed), kActionSpeedDown, kActionSpeedUp});
            page.buttons.insert(page.buttons.begin(), {ui::tr("Donate"), kActionDonate});
            if (empire_map) page.buttons.insert(page.buttons.begin() + 1, {ui::tr("Empire"), kActionEmpireMap});
            break;
        }
    }
    return page;
...
inline ui::Page promotion_page(const model::CityState& s, bool to_caesar) {
    ...
    if (to_caesar) {
        page.title = ui::tr("Rome offers you the throne");
        page.rows.push_back({ui::tr("Your ratings have earned"), ""});
        page.rows.push_back({ui::tr("the title of"), rank_name(20)});
        page.buttons.push_back({ui::tr("Accept"), kActionAccept});
    } else {
        page.title = ui::tr("Promotion");
        page.rows.push_back({ui::tr("Your ratings have earned you"), ""});
        page.rows.push_back({ui::tr("the rank of"), rank_name(rank + 1)});
        page.rows.push_back({ui::tr("and the province of"), province_name(g(s, 0x6CA4))});
        page.rows.push_back({ui::tr("Ratings average"), std::to_string(g(s, systems::administration::kAverage))});
        page.buttons.push_back({ui::tr("Accept"), kActionAccept});
        page.buttons.push_back({ui::tr("Wait 9 years"), kActionWait9});
        page.buttons.push_back({ui::tr("Wait 24 years"), kActionWait24});
    }
    return page;
}
...
inline ui::Page start_page(int funding_level, int difficulty, const std::string& name = "  Octavian  ") {
    ...
    page.rows.push_back({ui::tr("Governor"), trimmed(name)});
    const size_t level = static_cast<size_t>(std::clamp(funding_level, 0, 9));
    page.rows.push_back({ui::tr("Funding"), std::string(campaign::kFundingNames[level]) + ", " +
...
inline ui::Page ending_page(const model::CityState& s, bool caesar) {
    ui::Page page;
    if (caesar) {
        page.title = ui::tr("Ave, Caesar");
        page.rows.push_back({ui::tr("Rome hails you as"), rank_name(20)});
        page.rows.push_back({ui::tr("Last governed"), province_name(detail::g(s, 0x6CA6))});
        page.rows.push_back({ui::tr("Year"), year_text(detail::g(s, 0x6C32))});
    } else {
        page.title = ui::tr("Dismissed");
        page.rows.push_back({ui::tr("Three tributes to Rome in a row"), ""});
        page.rows.push_back({ui::tr("went unpaid."), ""});
        page.rows.push_back({ui::tr("Rome has ordered your arrest."), ""});
    }
    page.buttons.push_back({ui::tr("Continue"), kActionContinue});
    page.buttons.push_back({ui::tr("Quit"), kActionQuit});
    return page;
}
...
inline ui::Page battle_page(const model::CityState& s, const BattleView& b) {
    ...
    page.title = std::string(ui::tr("Battle against the")) + " " + race_name;
    ...
    page.rows.push_back({ui::tr("Cohort") + std::string(" ") + std::to_string(number + 1) + ", " +
                             (number >= 0 && number < 10 ? kCohortEmblems[static_cast<size_t>(number)] : "?"),
                         ""});
    page.rows.push_back({std::string("  ") + ui::tr("Regulars"), std::to_string(c.raw[military::kCohortRegulars])});
    page.rows.push_back({std::string("  ") + ui::tr("Irregulars"), std::to_string(c.raw[military::kCohortIrregulars])});
    page.rows.push_back({std::string("  ") + ui::tr("Auxiliaries"), std::to_string(c.raw[military::kCohortAuxiliaries])});
    page.rows.push_back({std::string("  ") + ui::tr("Morale"), std::to_string(c.raw[military::kCohortMorale])});
    page.rows.push_back({ui::tr("Barbarian army"), b.last.victory ? "destroyed" : std::to_string(a.raw[battle::kArmySize])});
    if (b.has_round) {
        page.rows.push_back({ui::tr("Last round - Romans"), std::to_string(b.last.romans)});
        page.rows.push_back({std::string("  ") + ui::tr("barbarians"), std::to_string(b.last.barbarians)});
    }
    if (ended) {
        page.rows.push_back({b.last.victory ? ui::tr("Victory !") : b.last.defeat ? ui::tr("The Cohort is destroyed") : ui::tr("You retreat"),
                             ""});
        page.buttons.push_back({ui::tr("Continue"), kActionContinue});
    } else {
        page.buttons.push_back({ui::tr("Tortoise"), kActionTactic + static_cast<int>(battle::Tactic::Tortoise)});
        page.buttons.push_back({ui::tr("Assault"), kActionTactic + static_cast<int>(battle::Tactic::Assault)});
        page.buttons.push_back({ui::tr("Flank"), kActionTactic + static_cast<int>(battle::Tactic::Flank)});
        page.buttons.push_back({ui::tr("Charge"), kActionTactic + static_cast<int>(battle::Tactic::Charge)});
        page.buttons.push_back({ui::tr("Retreat"), kActionRetreat});
    }
    return page;
}
