...
**Phases 0-5 are complete relative to what the RE corpus documents.** Phase 5's core RE question is answered and its toolbar UI is built. Read `GAIUS_ROADMAP.md`'s Phase 5 status block for the full account, including the two placement limits carried forward deliberately (drag auto-tiling, forum variant seeds).
...
Expect 1634/1634 checks passing, 1 skip (the `.SAV` corpus test, for the reason above). If this doesn't pass cleanly on a fresh clone, something regressed — investigate before adding new code. (The count jumped from 463 with Phase 5's toolbar work: the hit-test invariant is asserted across every button at every breakpoint, which is a lot of individual checks by design.)
...
Phase 5 is done. Three concrete RE residues remain from Phases 4-5, each small and well-bounded. The first two are listed with their handler addresses in `docs/CAESAR_CONSTRUCTION_DISPATCH_FINDINGS.md` section 14; the third is Phase 4's gap, newly unblocked by section 11:
...
3. **The other 21 of 22 `0x00`-`0x15` housing handlers** (Phase 4's gap — only tile 00 is traced). Section 11's full `DS:153A` extraction means every one of these handlers' addresses is now *known*; they just haven't been disassembled and read. Probably the cheapest of the three, and it's what would make housing actually grow and shrink.

Two asset-pipeline items, both newly narrowed in Phase 5 (see `docs/FORMATS.md`):

4. **The font sheets' PL8 pixel variant.** `FONT1`/`FONT2`/`ROMFONT`/`MINIFONT` parse their descriptors but decode to noise. **The palette is ruled out** as the cause — raw indices are a plausible 0..11 with index 0 dominant; the pixel path itself doesn't handle these sheets. Closing this swaps `ui/font.hpp`'s placeholder for the real typeface. One lead: font descriptors give 8-wide frames whose `x` origins step by 16.
5. **City-content sprite palette** — still the long-standing Phase 1/8 item. Note the *UI* side is fine: `PANEL1.VPX` + `PANEL1.P32` renders the real control panel correctly today.
...
One non-code item worth raising with the user: **the repo has no LICENSE file**, which is odd for a public open-source reimplementation. Picking the licence is theirs to decide. (SDL2's vendored Java glue is separately attributed in `android/README.md`.)
