...
> **Status: fully complete, 2026-09-08.** Implementation in `model/city_state.hpp`/`.cpp`, exercised by `tools/save_inspect.cpp`'s new actor-table dump. See verification notes under each item.
...
- [x] Round-trip: `CityState` → re-serialize (`model::serialize`) → byte-diff against the original save. **Verified against synthetic data only** — no real `.SAV` has ever been supplied (same gap noted since Phase 0/1; see `CLAUDE.md`). `tests/test_formats.cpp`'s `test_model_city_state_round_trip` builds an in-memory 57126-byte buffer with a deterministic non-repeating byte pattern, round-trips it, and byte-diffs the full output — plus spot-checks specific grid/actor positions against the *original* bytes directly (not just the round-tripped output), so a bug shared between `load()` and `serialize()` (e.g. a row/col transposition present in both) can't hide behind a trivially-passing round trip. This validates the block table is complete/correctly-ordered exactly as intended, independent of field *meaning* — but a real save is still needed before this can be called validated against the actual game, not just self-consistent.
- [x] Object/actor table normalized per the type-0..13 split (city-coordinate vs. province-coordinate) from `CAESAR_CITY_STATE_v9.md`. `model::Actor` wraps the full 50-byte record (most fields still opaque — only `screen_x/y`, `active`, `type`, `raw_x/y`, `packed_xy`, `state` have confirmed offsets) and exposes `coord_space()` (`ActorCoordSpace::City` for type < 11, `::Province` for type ≥ 11), matching the allocator's confirmed `packed_xy = row*100+col` vs. `row*40+col` split.

**Deliverable:** a save file editor's actual backend — load, inspect every field, re-save, confirmed byte-identical for untouched files (synthetic files, pending a real one). `save_inspect` now prints the active-actor table (type/coord-space/state/coordinates) alongside the existing block table and global-words dump, as a first concrete "inspect every field" consumer of the model.

**RE blockers (soft — affect fidelity, not the round-trip):**
- Save **loader** still not reverse engineered — the round-trip above only proves the *serializer's* block table is self-consistent, not that it matches what the original engine's (unreverse-engineered) loader actually does with a real save. Flag any byte-diff mismatch against a real save straight back into RE work the moment one is available.
...
> **Status: fully complete, 2026-09-08**, with one deliberate scope narrowing — see the building-handler item below. Implementation in `systems/service.hpp`/`.cpp`.
...
> **Status: complete within what the RE corpus actually documents, 2026-09-08** — which is narrower than this section originally assumed. Implementation in `systems/housing.hpp`/`.cpp`. See each item for exactly what is and isn't real.
...
- [x] `systems::housing::land_value_allows` — `CAESAR_CITY_STATE_v5.md` routine `0x2DB49`, implemented exactly: land value > threshold → tile becomes `0xA7`, coverage/`7BB4`/land-value reset to 0 at that cell. **"Tile-state transitions" (plural) turned out to be narrower than expected**: `CAESAR_CITY_STATE_v5.md` documents a per-tile dispatch table for all 22 IDs (`0x00`-`0x15`) but only traces **one** handler (tile `00`, routine `0x297AC`) in enough detail to implement — `systems::housing::tick_tile_00`. Its failure path ("checks the local A2C4 value and may replace the tile with C9, CA, etc.") has no documented thresholds, so it isn't implemented; flagged as open RE work in the code, not guessed at. Tiles `0x01`-`0x15` have known dispatch-table addresses but zero traced behavior — nothing to transcribe yet.
- [x] Population derivation — `systems::housing::population(squares, grade)`. The manual (`Caesar_1_Manual.pdf`, this project's copy — a condensed player's guide, not a full technical manual) confirms the *shape* only: density increases monotonically across the sixteen grades except "the fanciest houses actually have a slight drop in density." No numeric density table exists anywhere (checked the manual and the full RE corpus). `provisional_density_per_grade` therefore uses grade number itself as the density unit — the simplest function matching the confirmed shape, explicitly not a real game-balance number. Replace once real numbers turn up (a fuller manual edition, or RE work tracing the actual population routine).
- [x] Sixteen housing grades (`HousingGrade` enum, `Grade1`..`Grade16`) — implemented as a standalone Layer 3 concept, matching the manual's "sixteen grades of housing that a tent can evolve into." **Grade-to-tile-ID mapping is explicitly NOT attempted**, exactly as this checklist item and the RE blocker below already anticipated — `HousingGrade` has no relationship to `model::CityMap::tile` values yet.

**Deliverable:** narrower than originally scoped, honestly — a system that performs the *one* concretely-documented tile transition (tile 00's land-value gate) and derives population from a grade using the confirmed shape but placeholder numbers. Not yet "houses that grow/shrink/evolve" in general, since 21 of the 22 tile-00-through-0x15 handlers have no traced behavior to implement. That remains real, not-yet-started work — see RE blockers.

**RE blockers (hard):**
- The exact `43A5` tile-ID-to-housing-grade mapping is still provisional — unchanged from this phase's original framing, still waiting on Phase 5's construction-dispatcher work.
- **New, more specific blocker found while implementing:** tiles `0x01`-`0x15`'s individual handlers are dispatch-table stubs only (addresses known, behavior untraced) — closing this phase's original ambition (the *general* housing state machine, not just tile 00) needs each of those 21 handlers traced the way tile 00's was. This is a bigger remaining lift than the original checklist implied when it said "tile-state transitions... as recovered in `CAESAR_CITY_STATE_v5.md`" — that document only actually recovered one.
- No numeric population-density table exists anywhere in this project's sources (RE corpus or manual) — `provisional_density_per_grade`'s actual numbers are placeholders, not RE findings, and should not be treated as such by later phases.

---

## Phase 5 — Construction system & the dispatcher (the critical-path RE target)

> **Status: 6 of 7 checklist items complete, 2026-09-08. The core RE question — construction command → tile write → footprint — is SOLVED.** The one remaining item (toolbar UI) is not blocked on reverse engineering at all; it's blocked on the sprite/font pipeline gap that's been open since Phase 1. This took six passes; the history below is kept because the dead ends are as useful to a future session as the result.
>
...
- [x] Implement `systems::construction`. **Done for everything the RE supports**: `placement_spec` (per-command kind/seed tile/footprint), `can_place` and `place` (footprint + terrain-gate validation, seed-tile write, `7BB4` clear — matching the handlers' own `mov 3496:[bx+0x7bb4], 0`). Covers water (reservoir/well/fountain), tower, housing, temple, bath houses, hospital, school, oracle, theater, coliseum, hippodrome, plaza-adjacent civic buildings, barracks, prefecture, market and heavy industry. **Costs are still not wired** — the manual's Appendix A pricing and the located forum grade cost table (`3496:15A0`) were both left alone because there's no treasury in the engine yet to spend from; that belongs with Phase 7's economy work.
...
- [ ] Toolbar UI at the scalable-UI-factor from masterplan section 5a point 3, sized for the smallest target touch surface. **Still open, and now the only thing standing between Phase 5 and done.** Not blocked on RE any more — blocked on the *sprite/font pipeline*, which has been a known gap since Phase 1: `docs/FORMATS.md` records that no correct in-game sprite palette has been identified for city-content PL8 sheets, so there's no way to draw the real toolbar icons or text yet. Build mode currently reports the selected tool to stdout instead. Closing this properly means either finding that palette (Phase 1/8 follow-up) or deciding to ship a placeholder vector/geometric toolbar in the meantime — a product call, not an RE one.
...
>
> Three things stop this from being the *whole* deliverable, each for a stated reason rather than a shortcut: **roads and walls can't be built** (drag auto-tiling isn't reverse engineered — findings section 3), **the forum can't be built** (footprint known, per-variant seed tile not decoded), and **there's no toolbar** (sprite/palette gap, checklist item 7). So it's a playable slice of the civic-building game, not of the full game. Nothing here is faked to close the gap.
